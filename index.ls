angular.module \g0vis, <[ngRoute]>
  ...
  ..controller \prjDetailCtrl <[$scope $http]> ++ ($scope,$http) ->
  ...
  ..controller \prjCtrl <[$scope $http $routeParams]> ++ ($scope, $http, $routeParams) ->
    $scope <<< do
      panel: 0
  ...
  ..controller \ircCtrl <[$scope $http $routeParams]> ++ ($scope, $http, $routeParams) ->
    $scope <<< do
      panel: 2
    ...
    $http.get \https://kcwu.csie.org/~kcwu/ircstat/g0v-count.json .success (data) ->
      $scope.$broadcast \data.ready, data
      console.log data

  ..controller \ircCountCtrl <[$scope $element]> ++ ($scope, $element) ->
    $scope <<< do
  ...
  ..controller \ircCalendarCtrl <[$scope]> ++ ($scope) ->
    $scope <<< do
      line-chart: "M0,0"
      th: <[一 二 三 四 五 六 日]>
      event: do
  ...
  ..controller \ircRelationCtrl <[$scope]> ++ ($scope) ->
    $scope <<< do
      nodes: []
      links: []
